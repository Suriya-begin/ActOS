...
export default function SignUpPage() {
  ...
  const strengthScore = Math.min(Math.floor(password.length / 3), 4)
  const strengthColor = !password.length ? null : password.length < 8 ? "#ef4444" : password.length < 10 ? "#f59e0b" : "#10b981"
  ...
  return (
    <div className="min-h-screen bg-[#020408] flex overflow-hidden font-sans text-white relative selection:bg-cyan-500/30">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] rounded-full mix-blend-screen filter blur-[120px] opacity-30 animate-blob" 
        ...
        <div className="absolute bottom-[-10%] right-[-10%] w-[45vw] h-[45vw] rounded-full mix-blend-screen filter blur-[120px] opacity-20 animate-blob" 
      ...
      <div className="w-full flex flex-col lg:flex-row relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        >
          ...
          <h1 className="text-5xl lg:text-6xl font-extrabold tracking-tight mb-6 leading-[1.1]">
            Speak. <br/>
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-500">Execute.</span><br/>
            Dominate.
          </h1>
          <p className="text-lg text-slate-400 max-w-md leading-relaxed mb-12 font-light">
            Join the elite network of developers and creators controlling their digital environments through pure vocal intent.
          </p>

          <div className="space-y-6">
            {[
              { title: "Neuro-Linguistic Engine", desc: "Sub-second intent parsing via GPT-4o." },
              { title: "Quantum Security", desc: "Military-grade encryption and biometric verification." },
            ].map((feature, i) => (
              <motion.div 
                key={i}
              ...
              >
                <div className="mt-1 w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(34,211,238,0.8)]" />
                <div>
        ...
        <div className="flex-1 flex items-center justify-center py-12 lg:py-0 w-full">
          ...
          >
            {/* Glow behind form */}
            <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 to-cyan-500/20 blur-3xl rounded-[40px] -z-10" />
            ...
            <div className="bg-[#0a0e17]/80 backdrop-blur-2xl border border-white/[0.08] rounded-[32px] p-8 sm:p-10 shadow-2xl overflow-hidden relative">
              ...
              <div className="mb-8">
                ...
                <p className="text-sm text-slate-400 font-medium">Step into the future of computing.</p>
              </div>
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <motion.div variants={itemVariants} className="relative group">
                    <label className="block text-[11px] uppercase tracking-wider font-semibold text-slate-400 mb-2 pl-1 transition-colors group-focus-within:text-cyan-400">First Name</label>
                    <input 
                      type="text" required placeholder="Alex" 
                      value={firstName} onChange={e => { setFirstName(e.target.value); setError("") }}
                      onFocus={() => setFocusedInput('fname')} onBlur={() => setFocusedInput(null)}
                      className="w-full bg-white/[0.02] border border-white/[0.05] text-white text-sm px-4 py-3.5 rounded-xl outline-none focus:bg-white/[0.04] focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/10 transition-all placeholder-slate-600 font-medium" 
                    />
                  </motion.div>
                  <motion.div variants={itemVariants} className="relative group">
                    <label className="block text-[11px] uppercase tracking-wider font-semibold text-slate-400 mb-2 pl-1 transition-colors group-focus-within:text-cyan-400">Last Name</label>
                    <input 
                      type="text" placeholder="Chen" 
                      value={lastName} onChange={e => { setLastName(e.target.value); setError("") }}
                      onFocus={() => setFocusedInput('lname')} onBlur={() => setFocusedInput(null)}
                      className="w-full bg-white/[0.02] border border-white/[0.05] text-white text-sm px-4 py-3.5 rounded-xl outline-none focus:bg-white/[0.04] focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/10 transition-all placeholder-slate-600 font-medium" 
                    />
                  </motion.div>
                </div>
                ...
                <motion.div variants={itemVariants} className="relative group">
                  <label className="block text-[11px] uppercase tracking-wider font-semibold text-slate-400 mb-2 pl-1 transition-colors group-focus-within:text-cyan-400">Email Address</label>
                  <div className="relative">
                    <svg className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedInput === 'email' ? 'text-cyan-400' : 'text-slate-500'}`} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>
                    <input 
                      type="email" required autoComplete="email" placeholder="hello@example.com" 
                      value={email} onChange={e => { setEmail(e.target.value); setError("") }}
                      onFocus={() => setFocusedInput('email')} onBlur={() => setFocusedInput(null)}
                      className="w-full bg-white/[0.02] border border-white/[0.05] text-white text-sm pl-12 pr-4 py-3.5 rounded-xl outline-none focus:bg-white/[0.04] focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/10 transition-all placeholder-slate-600 font-medium" 
                    />
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="relative group">
                  <div className="flex justify-between items-center mb-2 pl-1 pr-1">
                    <label className="block text-[11px] uppercase tracking-wider font-semibold text-slate-400 transition-colors group-focus-within:text-cyan-400">Password</label>
                  </div>
                  <div className="relative">
                    <svg className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedInput === 'password' ? 'text-cyan-400' : 'text-slate-500'}`} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                    <input 
                      type={showPassword ? "text" : "password"} required autoComplete="new-password" placeholder="••••••••" 
                      value={password} onChange={e => { setPassword(e.target.value); setError("") }}
                      onFocus={() => setFocusedInput('password')} onBlur={() => setFocusedInput(null)}
                      className="w-full bg-white/[0.02] border border-white/[0.05] text-white text-sm pl-12 pr-12 py-3.5 rounded-xl outline-none focus:bg-white/[0.04] focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/10 transition-all placeholder-slate-600 font-medium tracking-[0.2em]" 
                    />
                  ...
                  {password.length > 0 && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex gap-1.5 mt-3">
                      {[0,1,2,3].map(i => (
                        <div key={i} className="flex-1 h-1 rounded-full transition-all duration-500 ease-out"
                          style={{ backgroundColor: i < strengthScore ? strengthColor! : "rgba(255,255,255,0.05)", boxShadow: i < strengthScore ? `0 0 8px ${strengthColor}40` : 'none' }} />
                      ))}
                    </motion.div>
                  )}
                </motion.div>
                ...
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-2xl blur-lg opacity-40 group-hover:opacity-70 transition-opacity duration-300" />
                  <div className="relative flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-sm text-white transition-all duration-300 bg-gradient-to-r from-cyan-500 to-blue-600 shadow-[inset_0_1px_rgba(255,255,255,0.2)] disabled:opacity-70 disabled:cursor-not-allowed">
                    {loading
                      ? <><svg className="animate-spin" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg> Initializing Protocol...</>
                      : <>Create Account <svg className="ml-1 group-hover:translate-x-1 transition-transform" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></>
                    }
                  </div>
                </motion.button>
              </motion.form>
              ...
              <div className="mt-8 text-center">
                <p className="text-xs text-slate-400">
                  Already a member?{" "}
                  <Link href="/auth/sign-in" className="text-white font-semibold hover:text-cyan-400 transition-colors relative after:absolute after:bottom-0 after:left-0 after:right-0 after:h-px after:bg-cyan-400 after:origin-right after:scale-x-0 hover:after:scale-x-100 hover:after:origin-left after:transition-transform after:duration-300 pb-1">
                    Authenticate here
                  </Link>
                </p>
              </div>
